...
## Last Session
- Date: 2026-09-06 (Windows host; workflow-tooling session, no application code)
- Worked on: bootstrap v32.5 UPDATE ONLY applied by hand from the updated `bootstrap.md` (extractor 65/65 OK; the retired `create-branch` Skill pair removed from `.agents/` and `.claude/`; the new `/update-workflow` Skill and `.cursor/bootstrap/workflow-source.py` landed; `.gitignore`, plan location and docs structure v23 all already compliant) → the first `/update-workflow` run (marker ABSENT → repair path; remote `main` of `kountlabs/Cursor-Bootstrap-Guide` at `242888796865` was byte-identical to the local bootstrap, so the provenance commit is the marker `.cursor/bootstrap/installed-version` = `32.5 242888796865` alone) → `/push` (commits `cacedf1` + `0b857cc`, CI run 34010813919) → `/document` (this block).
- Pattern worth knowing: (1) Under Git Bash, `gh api /repos/...` fails with "invalid API endpoint" because MSYS rewrites the leading-slash path — drop the leading slash (`gh api repos/...`) or set `MSYS_NO_PATHCONV=1` for the `gh` call ONLY; setting it shell-wide also breaks `/tmp/...` → Windows path conversion for `python`, so pass the helper Windows-native paths. (2) `/update-workflow` records a marker per run; from now on `/start-session`'s nudge reads it and a same-version content change on the source's `main` is reported as a release error, not a silent up-to-date. (3) `DeprecationWarning` lines in a CI job's stdout are NOT annotations — the Done-when for a runtime bump is the check-run annotation count (`gh api repos/<owner>/<repo>/check-runs/<job-id>/annotations`), not a log grep. (4) A plan find/replace run AFTER a peer block is transcribed rewrites the peer's verbatim quotation of the OLD text inside that block (it did once; restored from the peer's own output) — transcribe last, or scope replacements outside the Findings Log, and never re-run a replace script for its idempotence after transcription. (5) Codex usage limits are account-wide 5-hour windows weighted by model (astra ~150k tokens at xhigh vs sol 570k+ without tripping); a cut-off session resumes with `codex exec -s read-only resume <session-id> -m <model> -c model_reasoning_effort=<level> "<prompt>"` (sandbox flag BEFORE `resume`), skipping the re-read; preflight a new model id with a throwaway exec first (`gpt-6-astra` refused codex-cli 0.145.0).
