...
### Round 51 - 2026-09-18 - practitioner-profile Phase H Task H1 (Phases 1–5 as ONE surface, `git diff f1471f1..HEAD`), in-session `/review-loop` round 1 of cap 3
- Round status: Closed (4 of 4 Applied 2026-09-18 by Claude Code, the architect, in the same leg under gates=executor — MED-001's production code by the architect, the test units by the `loop-delegate` and verified hunk by hunk; composer-verified GREEN 2026-09-18 16:14 — ruff clean, mypy 34 files, pytest 2020)
- Source: Claude Code
- Baseline: primary review baseline `f1471f1` (the commit before this plan's first `/execute` checkpoint) → HEAD `bc6e2f0`, tree clean bar this plan at spawn. Read IN FULL: `speaker_embedding.py`, `practitioner_profile.py`, `enrolment.py`, `note_config.py`, `ui/models.py`, `ui/note.py`, `ui/practitioner.py`; read as FULL DIFFS against the baseline (the changed regions with their context): `transcription.py`, `note.py`, `speaker_eval.py`, `session.py`, `session_store.py`, `logging_setup.py`, `note_check.py`, `ui/transcript.py`, `ui/main_window.py`, `ui/microphone.py`, `docs/security/threat-model.md`, `docs/security/data-flow-map.md`, `docs/security/retention-schedule.md`, `docs/design-system.md`, `docs/testing/shipping-gate.md`, `docs/testing/speaker-measurement.md`, `AGENTS.md`, `CHANGELOG.md`, `scripts/measure-speakers.py`. Skipped with reason: the eleven test modules (+8,900 lines) were read by grep and targeted region reads, not in full — their pins are composer-verified GREEN at 2018 and this round's value is the cross-phase view; `scripts/loop-*`, `.claude/`, `.cursor/` are workflow tooling, out of scope. Suites NOT run (composer-run).
- Files reviewed — the cross-phase lenses: custody → attribution (`save_profile`/`load_profile` ↔ `attribution_readiness`/`attribution_inputs` ↔ `transcribe_session`'s identity check); attribution → UI (the D4 line, `_show_attribution_status`, the D2 reasons on both entry points); the D15 lease against every reader of the microphone; the consent gate across the tab, `learning_status` and the Note tab; the cue file from the packaged default through the loader, the provider factory, `append_user_cues` and the Practitioner tab; the docs against each of those structures.
- Validation basis: static reading; the ≤ 100-char rule to be re-confirmed by the composer's ruff run over the touched Python files.
- Finding verification: 9 candidates / 5 dropped / 0 downgraded. Dropped: (1) `enrolled_cluster` can confirm `speaker_2` in the matched case when every matched segment is textless — consistent with the ratified D4 rule (the best TEXTUAL cluster is confirmed; a no-match transcript auto-confirms below the threshold by the same design), not a defect; (2) `ATTRIBUTION_DID_NOT_RUN_REASON` could mislabel a transcript produced before enrolment — unreachable: the status renders once at `show_document`, and `begin_enrolment` is legal only outside the active states, so no shown document post-dates its own probe; (3) `attribution_inputs` has no `profile_root` seam, so `build_transcriber` always reads the default store — production has one store and every test injects the attribution callable; structural only, carried to H2; (4) `_write_learned_phrases` clears the queue before the write, so a failed write cannot be retried — edits are frozen after Save anyway and the failure is reported, by design; (5) a `speaker_1` cluster with no similarity in the degenerate branch — `enrolled_cluster` returns None and the document carries no fields, the documented contract.

#### MED-001: `desktop/src/scribe_desktop/ui/microphone.py:145-148` + `:287-290` — the 5 s model-status poll decrypts the voice profile every tick
- Triage: Fix-now
- Fix route: fix-on-fast
- Why it matters: cross-phase (custody → UI). The Phase-2 smoke-round-21 timer re-renders the model report every 5 s so a model file that appears on disk shows up; since Phase 3 that report includes the voice-profile line, so `refresh_model_status` → `models.model_report_lines` → `voice_profile_report_line` → `attribution_readiness` → `load_profile` runs a DPAPI unwrap, an AES-GCM decrypt and a parse of the biometric vector on the GUI thread every 5 s for the process lifetime — a periodic plaintext materialisation of the profile and a DPAPI call (SMB I/O under a redirected LOCALAPPDATA) that no document names: flow 12 says the report lines come from "a stat and one profile read". The profile is written only by the app itself (the Practitioner tab) so nothing external can appear the way a model file can; the poll buys nothing for that line.
- Current behaviour: the timer's slot recomputes all four lines, reading the profile each tick.
- Desired behaviour: the timer recomputes the three model-FILE lines (stats) only; the profile line is read at construction, on a device refresh and when the Practitioner tab reports a profile change, and cached in the widget between those reads.
- Pattern to follow: the `learned_phrases_changed` wiring (`ui/note.py` → `MainWindow` → `PractitionerScreen.refresh_learned_phrases`): the tab that changes the data says when; `models.model_report_lines` split into `model_file_report_lines` (stats) + `voice_profile_report_line` (the existing read).
- Pattern siblings: none found — `PractitionerScreen._availability_timer` (5 s) calls `refresh_availability`, two stats; `TranscriptScreen._show_attribution_status` reads once per `show_document`; `learning_status` reads at review start, at an add/move of a clinician line and at Save. Searched: `QTimer(`, `setInterval(`, `attribution_readiness(`, `load_profile(` across `desktop/src`.
- Invariant: no timer slot in the app calls `load_profile` (directly or through `attribution_readiness` / `voice_profile_report_line`); the profile is read by the microphone screen only from `refresh_profile_line`. AMENDED at peer round 55 PR-REG-006 (LEG 2, 2026-09-18): the poll's slot may call `refresh_profile_line` ONLY when the speaker model's presence (a stat) differs from its presence at the last read — one read per install or removal, zero while presence is unchanged — because the line's text folds that stat in.
- Verification: a pin that `MicrophoneScreen.refresh_model_status()` (the timer's slot) and `_on_benchmark_done` do not call `models.attribution_readiness` while `refresh_profile_line()` and `refresh_devices()` each call it once; a wiring pin that `PractitionerScreen.refresh_profile_state()` re-renders the microphone screen's line; `test_profile_reads_never_touch_the_default_root` unchanged and green; the suite.
- Regression risk: `MicrophoneScreen.refresh_model_status` callers (the timer, `refresh_devices`, `_on_benchmark_done`, `_on_benchmark_failed`, `test_profile_reads_never_touch_the_default_root`); `models.model_report_lines` callers (tests only after this fix); the report line goes stale for a profile deleted by hand outside the app until the next device refresh or restart — accepted (the tab itself does not poll either).
- /fix decision: Applied
- /fix notes: `ui/models.py` — `model_file_report_lines(kind)` split out (the three stat lines), `model_report_lines` now composes it with `voice_profile_report_line` (public API unchanged for its callers; `__all__` extended); `ui/microphone.py` — `self._profile_line` cached, `refresh_model_status` renders files + the cached line, new `refresh_profile_line` (one read + render), `refresh_devices` reads the profile line, construction reaches the read through `refresh_devices` only; `ui/practitioner.py` — `profile_changed = Signal()` emitted at the end of `refresh_profile_state`; `ui/main_window.py` — `profile_changed` → `microphone_screen.refresh_profile_line`; `docs/security/data-flow-map.md` flow 12 names the read points. Invariant verified by re-read: `load_profile` is reached from the microphone screen only through `refresh_profile_line`; the timer's slot and both benchmark handlers call `refresh_model_status`, which reads nothing. Pins (delegate): `test_model_status_poll_does_not_read_the_profile` (counts `models.attribution_readiness` calls: 1 at construction, 0 across two poll slots and a finished benchmark, +1 per `refresh_profile_line` / `refresh_devices`), `test_a_profile_change_on_the_tab_refreshes_the_microphone_report_line` (`@windows_only`: `save_profile` needs DPAPI). Sibling sites: none. NOT run (no grant): ruff, mypy, pytest.
- /fix date: 2026-09-18
- /fix applied by: Claude Code

- **[LOW]** LOW-001: `desktop/tests/test_ui_screens.py` — 22 of the 23 `MainWindow(` constructions pass `profile_root=tmp_path` but no `config_root`, so `PractitionerScreen.__init__` → `refresh_learned_phrases` → `load_learned_phrases(None)` reads the developer's REAL `%LOCALAPPDATA%\ClinikoScribe\config\section_cues.json` and sidecar in every one of them (the PR-REG-005 class, for the config store; the practitioner's real file holds learned phrases since the 2026-09-17 smoke) — surface: test-harness. Triage: Fix-now; Decision: Applied 2026-09-18 (the `loop-delegate`, verified by the architect) — `config_root=tmp_path / "config"` at all 22 sites (the `_rooted_window` helper already had it; every `MainWindow(` in the file now carries one) and the class pin: `test_profile_reads_never_touch_the_default_root` also forbids `note_config.default_config_root` and exercises `refresh_learned_phrases`. No count change.
- **[LOW]** LOW-002: `docs/security/threat-model.md` surface 9 (`:585-587`, `:605-608`) and `docs/design-system.md:51-53` — both say a READABLE profile's CURRENT record pre-ticks the consent box, but the tab keys its consent display, the learning-opt-in box and "Confirm consent" on `attribution_readiness`, i.e. readable AGAINST THE SHIPPED IDENTITY with the model file present; with the model absent or re-pinned the tab shows the D2 line, neither tick, and no way to change the opt-in, while `learning_status` (no identity check, documented) keeps the LAST SAVED opt-in in force on the Note tab until re-enrolment or Delete — the docs name the re-consent residual but not that the opt-in stays in force, and over-claim the pre-tick (claim outruns structure, safe direction) — surface: docs. Triage: Fix-now; Decision: Applied 2026-09-18 — the exact condition and the residual stated at both sites (class swept: `grep -n "READABLE" docs/` — the retention row's "readable profile carrying the CURRENT consent version" describes `learning_status`, which IS the plain read, so it stands); the code-side hardening (decouple the tab's consent display from attribution usability by carrying the plain-read profile beside the attributable one) is recorded for H3.
- **[LOW]** LOW-003: `desktop/src/scribe_desktop/speaker_eval.py:165` — `HARNESS_CONSENT_VERSION = "consent-v1"` reads as the current text version though `CONSENT_TEXT_VERSION` is `consent-v2`; attribution never reads the consent record, so the value is immaterial, but nothing said so (pinned by `test_speaker_eval.py:1585` as a fixed field) — surface: production (comment). Triage: Fix-now; Decision: Applied 2026-09-18 — the comment says the version is a pattern-satisfying placeholder that is deliberately not kept in step with the app's current text; the value and its test pin unchanged.

- Executor Judgment: none beyond the findings — no plan deviation found (Critical Constraints re-checked: the offline assertion before every ML import, no enrolment audio persisted, the DPAPI description binding, the windowed bound, D1's separation, D4 as ratified, D6/D7, D9 as amended, D14, compatibility).
- Structural Quality (review-only; carried to H2 as candidates, not findings here): (a) `note_config.py` at 1,666 lines holds the whole learning subsystem (`:979-1467`) beside the loader — a natural seam; (b) `ui/models.py` at 1,537 lines holds the practitioner/attribution composition (`:1209-1395`) beside the note-review view logic; (c) the embedder-identity triple is compared at two sites (`transcription.py:311-320`, `ui/models.py:1388-1393`); (d) `PractitionerScreen.refresh_devices` and `MicrophoneScreen.refresh_devices` repeat the device-combo population loop.
- Post-Fix Regressions: none — no fix precedes this round in Phase H; regression baseline: HEAD `bc6e2f0` (pre-`/fix` state).
- Missed-issue pass: re-read `practitioner_profile.py` (custody ordering, the key-present rule, the loader's error surface), `session.py`'s D15 block (`begin_enrolment` against `ACTIVE_STATES`, `end_enrolment` identity), `note_config.py:1303-1467` (the two writers and the deleter's prune), `ui/note.py:769-890` (the learner's gate order and the Save-time write), `ui/main_window.py` (the closed-signal ordering behind Task 5.6), `enrolment.py` (the failure slot and the post-shutdown checks); result: none beyond MED-001 (found on the second pass over `ui/microphone.py`'s timers).
- Deferral Confirmation Gate: silent — all four findings are Fix-now and auto-disposable under the headless contract (LOW/MED, do-the-work; MED-001 changes production code but hits no production-impact category: no dependency, env var, migration or deploy config).
- Cross-tool peer-review recommendation: not triggered by this round (0 CRIT, 0 HIGH); H4's codex pass is composer-seat regardless.
- Fix-delta self-check: PASS — the four production hunks (`ui/models.py`, `ui/microphone.py` ×4, `ui/practitioner.py` ×2, `ui/main_window.py`) and the comment re-read in context: `refresh_devices` is the only construction-time reader and runs after every widget exists; the unconnected `profile_changed` emit in `PractitionerScreen.__init__` is inert; the delegate's 114-line test diff read hunk by hunk (22 + 1 + 2), one gap (a DPAPI-backed test without `@windows_only`) caught and fixed in-leg; the three doc hunks re-read against the code; no drive-by edit; no touched Python line over 100 characters (the two longer lines the grep shows, `note.py:1348` and `session_store.py:475`, predate this leg and pass the baseline ruff).

### Round 52 - 2026-09-18 - practitioner-profile Phase H Task H1 (Phases 1–5 as ONE surface), in-session `/review-loop` round 2 of cap 3 (confirmation)
- Round status: Closed (no findings) — LOOP CONVERGED at round 2 of 3; H1 closes on this round
- Source: Claude Code
- Baseline: primary review baseline `f1471f1` → HEAD `bc6e2f0` plus the round-51 working-tree hunks (composer-verified GREEN: ruff clean, mypy 34 files, pytest 2020). Regression baseline: the pre-`/fix` state, HEAD `bc6e2f0`.
- Files reviewed: every round-51 hunk re-read fresh from disk (`git diff` over `ui/models.py`, `ui/microphone.py`, `ui/practitioner.py`, `ui/main_window.py`, `speaker_eval.py`, the three docs; the test diff at leg g1) plus every caller of the changed symbols (`model_report_lines`: tests only, signature unchanged; `refresh_model_status`: the timer, `refresh_devices` via `refresh_profile_line`, both benchmark handlers, two tests; `profile_changed`: the one main-window connection). The round-51 surface lenses re-applied to the unchanged files by memory of the leg-g1 read with spot re-reads of `practitioner_profile.py` and `note_config.py:1303-1467`.
- Post-Fix Regression Check: no signature, return or data shape changed — `model_report_lines` keeps its keyword-only signature and four-line output (`test_ui_models.py:250-260` still holds), `model_file_report_lines` is additive, `refresh_model_status` keeps its name and label; the one behavioural change is intended and pinned (the poll reads no profile; a tab action re-reads the line through the signal). The `profile_changed` emit at the end of `refresh_profile_state` fires once per tab action and once at construction before the connection exists — inert then, one extra profile read per tab action after, which is the designed cost. No caller behaves differently.
- Finding verification: 2 candidates / 2 dropped / 0 downgraded. Dropped: (1) "the report line goes stale for a profile deleted outside the app" — accepted at round 51 (the tab does not poll either; a device refresh re-reads); (2) "`refresh_devices` now reads the profile on every device refresh, twice at construction" — once at construction (the explicit `refresh_model_status()` call was removed) and once per refresh, as specified.
- Executor Judgment: none. Structural Quality: none new (the four round-51 candidates are H2's).
- Missed-issue pass: re-read `ui/microphone.py:64-80` (the cached line exists before any render), `:148-157` (construction order: timer, then `refresh_devices`), `:294-310` (the two renderers), `ui/practitioner.py:158-162` + `:508-511`, `ui/main_window.py:143-151`, and the three doc hunks against those lines; result: none.
- Deferral Confirmation Gate: silent (no candidates).
- Review History integrity: OK (52 + 52 at this round's checker run).

### Round 53 - 2026-09-18 - practitioner-profile Phase H Task H2 (`/simplify` over Phases 1–5 as ONE surface)
- Round status: Closed (1 of 1 Applied 2026-09-18 by Claude Code, the architect, in-leg under gates=executor — trivial, localised; the code hunks await the composer's suite run)
- Source: Claude Code simplify
- Scope: the round-51 surface (`git diff f1471f1..HEAD` plus the round-51 hunks), read at leg g1 in full / as full diffs; the four structural candidates carried from round 51 assessed first, then the rest of the surface under the conservatism guard. Baseline: HEAD `bc6e2f0` + the working tree.
- Finding verification: 7 candidates / 6 dropped as marginal or plan-chosen (listed under "Consciously left alone") / 0 downgraded.

#### SIMP-001: `desktop/src/scribe_desktop/transcription.py:1134-1143` + `desktop/src/scribe_desktop/ui/models.py:1405-1410` — the embedder-identity triple compared at two sites (severity: LOW)
- Triage: Fix-now
- Fix route: fix-on-fast
- Why it matters: "this profile was made by this embedder" (D16: `model_id`, `model_sha256` AND `embedding_dim` agree) was written out twice — the pipeline's refusal and the composition layer's fallback — so a fourth identity field, or a change to what agreement means, would have to be found at both sites; a duplicated invariant is the one class `/simplify` reserves above LOW, kept LOW here because the two sites are adjacent in intent and pinned by `test_attribution.py:626-648` and `test_ui_models.py` `TestAttributionInputs`.
- Current behaviour: two hand-written three-way comparisons.
- Desired behaviour: one method, `PractitionerProfile.made_by(embedder) -> bool`, read by both sites; behaviour identical (the same three equalities, the same error / fallback).
- Pattern to follow: `note.spoken_by_confirmed_clinician` — the ownership test written once and read by the router and the learner.
- Pattern siblings: none found — `load_profile`'s identity check (`practitioner_profile.py:344-351`) compares the two ids WITHOUT the dimension by design (the caller supplies ids, not an embedder; the dimension is checked once the embedder exists) and raises the structural `model` reason, so it is not the same predicate. Searched: `model_sha256 !=`, `embedding_dim !=`, `model_id !=` across `desktop/src`.
- Verification: `test_practitioner_profile.py` `TestModel.test_made_by_compares_the_whole_embedder_identity` (the four cases: agree, id, digest, dimension); the existing pipeline refusal tests (`test_attribution.py:626-648`) and `TestAttributionInputs` unchanged and green.
- Regression risk: `transcribe_session` (the refusal message unchanged), `attribution_inputs` (the fallback unchanged); `practitioner_profile` gains a `TYPE_CHECKING`-only import of `speaker_embedding.SpeakerEmbedder` (no runtime import, no cycle: `speaker_embedding` imports only `benchmark`).
- /fix decision: Applied
- /fix notes: `practitioner_profile.py` — `PractitionerProfile.made_by` + the `TYPE_CHECKING` import; `transcription.py:1134-1140` and `ui/models.py:1405-1407` read it; the new test. Verified by re-read of both sites and the diff. NOT run (no grant): ruff, mypy, pytest.
- /fix date: 2026-09-18
- /fix applied by: Claude Code

- Consciously left alone (the conservatism guard applied): (a) splitting the learning subsystem (`note_config.py:979-1467`) into its own module — the two writers and the deleter validate through the loader's own private helpers (`_read_config_blob`, `_parse_config_blob`, `_write_config_file`) and the module docstring's "the ONE config file the app writes itself" is a locality property worth keeping; a split would export those privates and rename ~20 references across the security docs, `AGENTS.md` and the plan on a peer-converged surface for organisational gain only — revisit at the Phase 3B boundary, which reshapes this module anyway; (b) moving the practitioner/attribution composition (`ui/models.py:1209-1411`) out of `ui/models.py` — that module is the ONE GUI-free view-logic module by design (its docstring; every screen's logic lives there), so a per-feature split is a broader restructure of a chosen shape, not a simplification; (c) the repeated device-combo population in `PractitionerScreen.refresh_devices` and `MicrophoneScreen.refresh_devices` — the two loops keep different state (device names for the profile vs the failure latch) and a shared helper would save ~10 lines at the cost of parameters, marginal; (d) the three norm guards (`speaker_embedding._l2_normalised`, `transcription._unit_vector`, `enrol`'s mean check) — three different contracts (a typed model error, a `ValueError`, a numeric fallback in `_cosine`), not one; (e) the three derivations of `admissible_sections` from a segment (`ui/models.py` `eligible_utterances` / `editable_lines.allowed_for`, `ui/note.py` `_allowed_sections`) — one call each with the same four arguments, a helper would save little; (f) an `attribution_inputs` `profile_root` seam — would ADD a parameter, not remove one (the round-51 drop stands).
- Summary: 1 LOW (Fix-now, applied) / 0 substantial — no simplification needs a scoped `/review-plan`; round 53.

### Round 54 - 2026-09-18 - practitioner-profile Phase H Task H3 (`/security-review` over Phases 1–5 as ONE surface)
- Round status: Closed (1 of 1 Applied 2026-09-18 by Claude Code, the architect, in-leg under gates=executor — a test-only pin; awaits the composer's suite run)
- Source: Claude Code security-review (the portable threat-model checklist, run in this session — no native security reviewer was invoked)
- Scope: the round-51 surface, with the spawn prompt's focus — DPAPI custody of the profile key and its description binding, the consent gate and version check, the D4 residual, the cue-file input surface (`section_cues.json` + the learned sidecar), the refusal filter as the enforcing control — plus the code-side consent/opt-in decoupling recorded at round 51 LOW-002. Trust boundaries examined: the same-user boundary (boundary 2) around the profile store and the config directory; the GUI-thread reads; the worker-thread attribution path; the log tripwire. Baseline: HEAD `bc6e2f0` + the working tree.
- Finding verification: 3 candidates / 2 dropped / 0 downgraded. Dropped: (1) "a hand-edited sidecar key with bidi controls renders on the Practitioner tab" — traced false: `load_learned_phrases` keeps a sidecar entry only when its normalised phrase is in the CUE FILE and renders the cue file's validated phrase (`note_config.py:1418-1422` → `hit[1]`), and the list item's data is that same phrase, so `delete_user_cue` never receives a sidecar key; (2) "a profile blob wrapped for a session placed in the profile directory reads as the profile" — traced false: `unwrap_key_from_file` verifies the stored description (`session_store.py:589-590`) and `save_profile` refuses to write over a present key it cannot unwrap (`practitioner_profile.py:263-268`), fail-closed both ways.

#### SEC-001: `desktop/src/scribe_desktop/ui/models.py:1219-1249` — "a changed consent text is a new version" had no enforcing structure (severity: MED)
- Triage: Fix-now
- Fix route: fix-on-fast
- Why it matters: the consent control rests on `consent_is_current` — a string equality between a record's `consent_text_version` and `CONSENT_TEXT_VERSION`. The docs (`AGENTS.md` subsystem pointer, threat-model surface 9, the retention row) claim "a changed text is a new version", but nothing bound the shipped text to its version: an edit to `CONSENT_TEXT_V2` that kept `"consent-v2"` (a wording "improvement", a merge) would make every stored v2 record read as current consent to words the practitioner never saw — the recurring claim-outruns-structure class, here on the consent artefact itself. The threat is a same-codebase change, not an attacker; the blast radius is consent integrity for every profile.
- Current behaviour: `test_consent_text_is_versioned` pinned the version string, the trailing marker and seven promise substrings — a paraphrase would pass.
- Desired behaviour: the ratified text is pinned VERBATIM beside its version, so changing the text without ratifying and pinning a v3 fails the suite; the docstring names the procedure (ratify v3, add `CONSENT_TEXT_V3`, bump `CONSENT_TEXT_VERSION`, pin the new text, never edit v2).
- Pattern to follow: the shipped-cue-file pins (`test_note_config.py`: `DEFAULT_SECTION_CUES` equals the packaged file, exact counts) — a ratified artefact pinned byte-for-byte.
- Pattern siblings: none found — `CONSENT_TEXT_V1` is history, never rendered (`ui/practitioner.py` shows `CONSENT_TEXT_V2` only) and no record carrying v1 is current, so a change to its text enables nothing; `LEARNING_OPT_IN_LABEL` is already pinned verbatim (`test_ui_models.py:631-634`). Searched: `CONSENT_TEXT`, `consent_text_version`, `consent_is_current` across `desktop/`.
- Verification: `test_ui_models.py` `TestPractitionerReportLines.test_the_current_consent_text_is_the_ratified_text_verbatim` — the literal is the plan's ratified blockquote (`Consent text v2`, 2026-09-16) with the markdown emphasis on "you" dropped as the shipped label drops it; the suite.
- Regression risk: none at runtime (test-only); a future text change now needs the procedure the docstring names.
- /fix decision: Applied
- /fix notes: `desktop/tests/test_ui_models.py` — one test added after `test_consent_text_is_versioned` (which stays). Verified: the literal transcribed from `ui/models.py:1236-1249` fragment by fragment (boundary spaces, the em dash, the trailing "Version consent-v2."). NOT run (no grant): ruff, mypy, pytest.
- /fix date: 2026-09-18
- /fix applied by: Claude Code

- Checked and clean (the categories examined, with the trace): **Crypto & custody** — the profile key is DPAPI-wrapped current-user with a profile-specific description that the unwrap verifies (`session_store.py:547-591`), the blob is AES-256-GCM under a versioned AAD (`practitioner_profile.py:83, 274, 305`), key-first write, reusable-key re-enrolment, key-first delete (`:243-359`), every unusable state typed and structural with the input hidden from validation errors (`:118-124, :312-327`); the same-user residuals are named in surface 1. **Consent gate & version** — the Record button needs the tick (`ui/practitioner.py:530-535`), the tick comes only from a current record readable against the shipped identity (`:454-488`, the round-51 LOW-002 wording), learning needs a readable profile + current version + opt-in re-read at review start, at every clinician-line add/move and at Save (`ui/note.py:434-441, :783, :854`); the version binding is now pinned (SEC-001). **The D4 residual** — practitioner-ratified, unconditional, visible similarity, one-click change, Generate's other gates untouched (`ui/transcript.py:619-630, :656-672`); a responsibility boundary, not re-flagged. **The cue-file input surface** — `SectionCuesFile` refuses unknown keys (`Literal`), a wrong `schema_version`, control / line-separator / bidi characters and blank text per phrase (`_TriggerText`, `note_config.py:202-233`), phrases with no content tokens and normalised duplicates within or across sections (`:724-749`); cue phrases never enter a note (they select verbatim transcript utterances; the ownership rule still applies to clinician-owned sections); the learned sidecar is read only by the tab, its keys never rendered or acted on (dropped candidate 1), and a malformed sidecar fails loud BEFORE the cue write (`:1378` precedes `:1382`), so nothing is written. **The refusal filter** — one implementation at one site (`refuse_learning_candidate`), source tokens in original case, the two exemption lists consulted at the real first word for the name class only (`:1131-1146`), class-pinned; the shape-only limit and the homograph residual are documented (surfaces 1 and 10). **The round-51 LOW-002 decoupling** (the tab's consent display keyed on attribution usability while `learning_status` honours the plain record) — assessed and NOT logged as a finding: the saved consent is honoured in the safe direction (what the practitioner ticked stays in force), Delete withdraws it in one click, the state (model file removed or re-pinned after enrolment) is rare and now documented at surface 9; the code-side decoupling stays a recorded hardening, to be taken up if Phase 6's measurement or a model re-pin makes the state common. **Secrets & logging** — no module on the profile or learning path logs; `TaskThread` never logs; every label that renders exception or config text is `PlainText`; the tripwire markers cover the profile models. **Injection / taint** — no shell, SQL, template or path sink takes practitioner or transcript text; config file names are constants; the profile root and config root are derived, never user-typed. **Network** — none at runtime; the offline kill-switches are asserted before every ML import (`speaker_embedding.py:119-125, :285`, `enrolment.py:99-103`). **Dependencies & config** — no new dependency, environment variable or deploy-side change in the surface.
- Summary: 0 CRIT / 0 HIGH / 1 MED (Fix-now, applied, test-only) / 0 LOW; no substantial hardening to route; round 54. The project's own security tests (the tripwire fixtures, the no-sockets integration legs, the custody suites) are the composer's next run.

### Round 55 - 2026-09-18 - practitioner-profile Phase H Task H4, independent cross-family codex peer review (hardening-stage re-check, pass stage-6.p1)

- Round status: Closed (2 of 2 Applied 2026-09-18 16:45 by Claude Code, the architect, in LEG 2 under gates=executor after the composer's auto-disposition; LEG 1 verified 16:35: PR-LOW-041 LOW test-harness across three files, PR-REG-006 LOW production, a fix-induced regression of round 51 MED-001; the hunks await the composer's suite run before codex confirmation round 56)
- Source: Codex peer-review
- Baseline: HEAD `bc6e2f0b72ccc830a1d500d45a6e3bd827104659`; reviewed the uncommitted hardening-stage hunks against HEAD.
- Files reviewed: All hunks in `git diff -- desktop docs`: seven production modules, three test modules and three documentation files. Additional targeted reads covered timer dependencies, profile custody, note learning, `test_status_and_app.py`, `test_integration_no_sockets.py` and test fixtures. Plan reading restricted to Critical Constraints and rounds 51–54.
- Validation basis: Static source/diff review, repository-wide caller searches and read-only AST inspection. No project imports, suite execution or filesystem writes. Composer-reported results—ruff clean, mypy clean over 34 files, pytest 2022 passed—were not independently rerun.
- Finding verification: 4 candidates / 2 dropped / 0 downgraded. Retained: incomplete test-store isolation and model-dependent cached status. Dropped: construction-order signal hazard and learning after consent withdrawal.

#### Confirmed / disputed (rounds 51–54)

- **MED-001 — partly confirmed:** The no-timer-profile-read invariant holds at `desktop/src/scribe_desktop/ui/microphone.py:294`; profile mutations refresh the report through `desktop/src/scribe_desktop/ui/main_window.py:150`. However, model availability is another dependency of the cached line: PR-REG-006 below.
  - LEG 1 verified (architect, 2026-09-18 16:35): the remark is CORRECT — the cached line's text depends on two inputs, the profile read AND the speaker-model stat (`ui/models.py:1206-1211`: `attribution_readiness` folds `speaker_embedder_available` into `reason`), and round 51 cached the composite; the model-file half was the very thing the smoke-round-21 poll exists to keep fresh. Disposed as PR-REG-006 below.
- **LOW-001 — partly confirmed:** All 23 `MainWindow` constructions in `test_ui_screens.py` now supply temporary config roots. The pin at `desktop/tests/test_ui_screens.py:2756` patches the actual default resolver used by `desktop/src/scribe_desktop/note_config.py:1400`; its `AssertionError` is not swallowed by the tab’s `NoteConfigError` handler. Other constructor paths remain unisolated: PR-LOW-041 below.
  - LEG 1 verified (architect, 2026-09-18 16:35): the remark is CORRECT — round 51's sibling search was `MainWindow(` in one file; the `PractitionerScreen` helper and the two other modules' window constructions were not searched (the round-51 class was under-enumerated, the 🔁 tail this rule exists to prevent). Disposed as PR-LOW-041 below.
- **LOW-002 — confirmed:** `docs/security/threat-model.md:585` and `docs/design-system.md:51` correctly qualify consent pre-ticking by shipped identity/model presence, matching `desktop/src/scribe_desktop/ui/practitioner.py:456`. The saved-opt-in residual at `docs/security/threat-model.md:611` matches the independent plain-profile read at `desktop/src/scribe_desktop/ui/models.py:1050`. No additional unsupported claim found in the changed sentences or sibling descriptions.
- **LOW-003 — confirmed:** The comment at `desktop/src/scribe_desktop/speaker_eval.py:163` accurately describes the synthetic version used at `:846`; transcription does not inspect that consent record.
- **SIMP-001 — confirmed:** `desktop/src/scribe_desktop/practitioner_profile.py:216` retains exactly the three identity equalities. Their negation replaces the former disjunction without changing the exception at `desktop/src/scribe_desktop/transcription.py:1139` or fallback at `desktop/src/scribe_desktop/ui/models.py:1405`. The import at `desktop/src/scribe_desktop/practitioner_profile.py:81` is type-checking-only; no runtime import cycle is introduced.
- **SEC-001 — confirmed:** Read-only AST reconstruction found identical UTF-8 content: 888 bytes for the shipped value and pinned literal. The independent literal at `desktop/tests/test_ui_models.py:645` and equality assertion at `:659` would fail on a one-word production-text edit; the version is separately pinned at `:660`.

#### Timer, caller and cross-phase verification

- **Every QTimer slot:** `MicrophoneScreen._poll_level` (`desktop/src/scribe_desktop/ui/microphone.py:144`), `MicrophoneScreen.refresh_model_status` (`:151`), `PractitionerScreen.refresh_availability` (`desktop/src/scribe_desktop/ui/practitioner.py:367`), `SessionScreen._watch_state` (`desktop/src/scribe_desktop/ui/session_screen.py:102`) and `app.periodic_sweep` (`desktop/src/scribe_desktop/app.py:182`). Their downstream paths cover microphone levels, model stats, widget enablement, session state and session sweeping/relisting; none reaches a profile read.
- **Every profile-read caller:** `load_profile` is called by `learning_status` (`desktop/src/scribe_desktop/ui/models.py:1055`) and `attribution_readiness` (`:1363`). Readiness is reached by `voice_profile_report_line` (`:1206`), `attribution_inputs` (`:1397`), the Practitioner readiness provider (`desktop/src/scribe_desktop/ui/practitioner.py:202`) and Transcript’s default readiness callback (`desktop/src/scribe_desktop/ui/transcript.py:99`, invoked at `:309`). `voice_profile_report_line` is called by `model_report_lines` (`desktop/src/scribe_desktop/ui/models.py:1171`) and `MicrophoneScreen.refresh_profile_line` (`desktop/src/scribe_desktop/ui/microphone.py:309`).
- **Signal ordering:** The microphone screen exists first (`desktop/src/scribe_desktop/ui/main_window.py:105`). Practitioner construction emits before the connection is installed, so that emission is inert; the microphone has already initialized its own cache. Subsequent emissions occur after state and controls update (`desktop/src/scribe_desktop/ui/practitioner.py:510`). Flow 12’s new read-point description matches this wiring.
- **Strongest bounded cross-phase check:** Consent withdrawal during an open review cannot flush previously queued phrases. Key-first deletion is enforced at `desktop/src/scribe_desktop/practitioner_profile.py:372`; Save commits the note before learning (`desktop/src/scribe_desktop/ui/note.py:1081`, `:1092`), and learning rereads consent immediately before the synchronous append (`:854`, `:864`). This candidate was dropped.

#### PR-LOW-041 — [LOW] `desktop/tests/test_ui_screens.py:705` — Default config reads remain through sibling test constructors

- Triage: Fix-now
- Fix route: fix-on-fast
- Why it matters: LOW-001’s test-isolation problem remains reachable. These tests read the developer’s learned phrases and depend on machine-local config contents.
- Current behaviour: `_practitioner_screen` supplies a temporary profile root but omits `config_root` from its default kwargs. Twenty callers omit an override. Construction unconditionally calls `refresh_learned_phrases` (`desktop/src/scribe_desktop/ui/practitioner.py:373`), which reaches `default_config_root` through `load_learned_phrases(None)` (`desktop/src/scribe_desktop/note_config.py:1400`). Two additional `MainWindow` paths omit the config root.
- Desired behaviour: Default the helper’s config root to `tmp_path / "config"` while preserving explicit overrides; supply temporary config roots to both remaining window paths. Isolate the subprocess’s profile root as well.
- Pattern to follow: The rooted window helper at `desktop/tests/test_ui_screens.py:4525` and default-root poison pin at `:2756`.
- Pattern siblings: Searches for `MainWindow(`, `PractitionerScreen(`, `_practitioner_screen(`, `config_root`, `LOCALAPPDATA` and `default_config_root`, plus AST enumeration, found:
  - `_practitioner_screen` callers without overrides in `desktop/tests/test_ui_screens.py` at lines 787, 792, 800, 814, 833, 895, 917, 938, 972, 995, 1015, 1035, 1057, 1081, 1107, 1120, 1145, 1183, 1207 and 1225.
  - `desktop/tests/test_status_and_app.py:67`.
  - The child-program constructor at `desktop/tests/test_integration_no_sockets.py:215`, which also omits `profile_root` and inherits the environment.
- Invariant: UI tests resolve practitioner config/profile reads beneath temporary test roots.
- Verification: Statically traced construction → learned-phrase refresh → default resolver; no overriding fixture or environment redirection covers these paths. Extend the poison-root coverage to these constructor paths, including the child program.
- Regression risk: Test-only. Preserve callers’ explicit config-root overrides; application defaults need no change.
- /fix decision: Pending
- LEG 1 verified (architect, 2026-09-18 16:35): materiality=behavioral severity=verified: LOW surface=test-harness — evidence: `desktop/tests/test_ui_screens.py:705-713` — the `_practitioner_screen` helper's default kwargs carry `profile_root` and a local readiness provider but no `config_root`, so every caller that passes no override (the twenty lines the peer lists; `:787`, `:792`, `:800` re-read and confirmed) constructs a tab whose `__init__` calls `refresh_learned_phrases()` (`ui/practitioner.py:373`) → `load_learned_phrases(None)` → `default_config_root()` (`note_config.py:1400`) — the developer's real cue file and sidecar, read at construction. The two window paths confirmed: `desktop/tests/test_status_and_app.py:67-72` passes `sessions_root` and `profile_root` but no `config_root`; the no-sockets child program at `desktop/tests/test_integration_no_sockets.py:215` passes `sessions_root` only, so that child process reads the developer's REAL profile (a DPAPI unwrap of the biometric blob) and real config at construction — the same class, one module wider than the peer's phrasing. `NoteScreen()` without a root (`:3037`, `:3078`, `:3261`) is NOT a sibling: the Note tab reads config only at a Save with learning on, and those tests construct it with no learning provider. rec=Fix-now — test-only, three files: default `"config_root": tmp_path / "config"` in the helper (overrides preserved by `kwargs.update`), `config_root=tmp_path / "config"` at `test_status_and_app.py:67`, and in the child program derive both roots from the same `tempfile.mkdtemp()` parent (`profile_root=root.parent / 'profile'`, `config_root=root.parent / 'config'`); extend the poison-root pin's class by asserting, in one new `TestPractitionerScreen` test, that a helper-built tab under the `note_config.default_config_root` poison constructs and refreshes cleanly. No production change; no count change beyond the one new test.
- /fix decision: Applied
- /fix notes: `desktop/tests/test_ui_screens.py` — the `_practitioner_screen` helper's default kwargs gain `"config_root": tmp_path / "config"` (overrides still win through `kwargs.update`; the 20 override-free callers now construct off a temp config root); new `TestPractitionerScreen.test_helper_built_tab_never_reads_the_default_stores` (both default resolvers poisoned; construction, `refresh_profile_state` and `refresh_learned_phrases` clean; the note label reads the empty-state text). `desktop/tests/test_status_and_app.py:72` — `config_root=tmp_path / "config"`. `desktop/tests/test_integration_no_sockets.py:212-220` — the child program derives `base = Path(tempfile.mkdtemp())` and passes `sessions_root=base / 'sessions'`, `profile_root=base / 'profile'`, `config_root=base / 'config'`, so the child never unwraps the developer's real profile. Sibling sites: the three the peer enumerated, all swept; `NoteScreen()` sites confirmed non-siblings (no config read without a Save under learning). Verified by re-read of each hunk. NOT run (no grant): ruff, mypy, pytest.
- /fix date: 2026-09-18
- /fix applied by: Claude Code

#### PR-REG-006 — [LOW] `desktop/src/scribe_desktop/ui/microphone.py:300` — Cached attribution message stays stale after speaker-model installation

- Triage: Fix-now
- Fix route: fix-on-fast
- Why it matters: Following the displayed model-installation remedy can leave the report contradicting itself indefinitely: the speaker-model line says installed while the cached line still says voice attribution is off because the model is absent.
- Current behaviour: With an existing readable profile and a missing speaker model, `attribution_readiness` returns `SPEAKER_MODEL_MISSING_REASON` (`desktop/src/scribe_desktop/ui/models.py:1373`). Construction caches that rendered result. Installing the model changes the polled file line, but `refresh_model_status` continues displaying the old profile line until an explicit profile/device refresh. Before this change, the poll refreshed both. Round 51 accepted hand deletion of the profile, not this independent model-file dependency.
- Desired behaviour: Reflect model-presence changes without periodic profile decryption. Cache a non-sensitive, model-independent profile summary at the authorized read points and combine it with current model stats when rendering.
- Pattern to follow: The stat-only model reporting at `desktop/src/scribe_desktop/ui/models.py:1119`; preserve MED-001’s prohibition on timer-triggered profile reads.
- Pattern siblings: Searches of profile reporting/readiness and refresh callers found the composed report at `desktop/src/scribe_desktop/ui/models.py:1151` and the Practitioner tab’s separately cached readiness at `desktop/src/scribe_desktop/ui/practitioner.py:451`. The composed report reads fresh. The Practitioner tab’s model-absence limitation was explicitly assessed in round 54 and is not reopened here. Both benchmark handlers also render the microphone’s stale cache (`desktop/src/scribe_desktop/ui/microphone.py:368`, `:374`).
- Invariant: A model-file availability change must not leave a current-status message asserting the opposite model state; timer rendering must still perform zero profile reads.
- Verification: Statically traced existing profile + missing model → cached fallback → model installation → file-only poll. Add coverage for model absence/presence transitions while asserting unchanged profile-read counts; the current poll test covers an absent profile only.
- Regression risk: Limited to status reporting; transcription independently rereads readiness at `desktop/src/scribe_desktop/ui/models.py:1397`. Avoid retaining the biometric vector merely to render the cache.
- /fix decision: Pending
- LEG 1 verified (architect, 2026-09-18 16:35): materiality=regression (fix-induced, round 51 MED-001) severity=verified: LOW surface=production — evidence: `ui/models.py:1199-1217` — `voice_profile_report_line` returns `readiness.reason`, which for a readable profile with the model FILE absent is `SPEAKER_MODEL_MISSING_REASON` (set at `attribution_readiness`, the `speaker_embedder_available(kind)` branch); `ui/microphone.py:294-301` renders that cached string on every poll and `:303-310` is the only re-read, reached from `refresh_devices` and the tab's `profile_changed` — so after the practitioner runs `setup-models.py --only speaker-embedding` (the remedy the line itself names) the file line reads "installed - verified when it loads" while the profile line keeps saying the model is not installed until a device refresh, a tab action or a restart; before round 51 the poll re-derived both, and smoke round 21 made "never permanently stale after setup-models ran" the report's property. Severity LOW: status text only, self-healing on any refresh, and the worker re-reads readiness at transcription (`ui/models.py:1397`), so no consultation runs without attribution because of it. rec=Fix-now — the bounded shape: `refresh_model_status` (the poll) takes the speaker-model stat it already takes for the file line and, when that presence DIFFERS from the presence recorded at the last profile read (a new `self._profile_line_model_present: bool | None`), calls `refresh_profile_line` — zero profile reads while presence is unchanged, ONE read per real transition (install or removal), never periodic; the MED-001 invariant is restated as "the poll reads the profile only on a speaker-model presence transition" in the two microphone docstrings, flow 12 and the round-51 block's Invariant line; pin by extending `test_model_status_poll_does_not_read_the_profile`: monkeypatch `models.speaker_embedder_available` to a switchable stub, assert the read count is unchanged across polls at steady presence and +1 on a flip (both directions). The peer's alternative — cache a non-sensitive summary and re-render it against live stats — is acceptable if the composer prefers a strictly zero-read poll, at the cost of splitting `voice_profile_report_line` into a read half and a render half; the transition-gated read is the smaller change and keeps one renderer.
- /fix decision: Applied
- /fix notes: `desktop/src/scribe_desktop/ui/microphone.py` — `self._profile_line_model_present: bool | None` recorded by `refresh_profile_line` from `models.speaker_embedder_available()`; `refresh_model_status` takes the same stat and, when it differs from the recorded presence, calls `refresh_profile_line` (one read, then the render) and returns — otherwise it renders the file lines plus the cached line as before; both docstrings and the attribute comment restated. `desktop/src/scribe_desktop/ui/models.py` — `model_report_lines`'s docstring names the transition read point. `docs/security/data-flow-map.md` flow 12 — the same. The round-51 MED-001 Invariant line amended in place. Pin: `test_model_status_poll_does_not_read_the_profile` extended with a switchable `models.speaker_embedder_available` stub — steady polls and a finished benchmark read nothing; an install flips the file line to "installed" and costs exactly one read; two steady polls read nothing; a removal costs one more read; the speaker-model line asserted both ways. Invariant verified by re-read: the poll's slot reaches `voice_profile_report_line` only through the transition branch, and the recursion terminates because `refresh_profile_line` records the presence before re-entering the renderer. Regression risk re-read: `refresh_devices`, both benchmark handlers and the tab signal unchanged. NOT run (no grant): ruff, mypy, pytest. Composer suites over the leg: ruff clean, pytest 2023, mypy strict caught an unexported name (`ui.models.speaker_embedder_available`, read by the microphone screen through the module so the test's one monkeypatch reaches the transition check and the readiness probe alike) — fixed in leg g5 by adding it to `ui/models.py`'s `__all__` (the `default_sessions_root` re-export precedent; the round-51 discipline).
- /fix date: 2026-09-18
- /fix applied by: Claude Code

PEER-ROUND-55 RESULT: 2 findings (CRIT 0 / HIGH 0 / MED 0 / LOW 2).

- Fix-delta self-check: PASS — re-read the seven applied hunks across six files (`ui/microphone.py` ×3, `ui/models.py` ×1, `docs/security/data-flow-map.md` ×1, `test_ui_screens.py` ×3, `test_status_and_app.py` ×1, `test_integration_no_sockets.py` ×1): the new test's local import of `NO_LEARNED_PHRASES_TEXT` was missing on the first write and added in-leg (flagged-and-fixed in-leg: a `NameError` the suite would have caught); the child-program string keeps every line under 100 characters; no drive-by edit; the round-51 hunks otherwise untouched.

Cap verdict: accept — production-behavioral — both findings LOW at their labels (one a fix-induced regression of round 51 with a bounded transition-gated fix, one the round-51 test-isolation class completed across three files); nothing CRIT/HIGH, nothing to defer; 1 of 5 rounds spent.

### Round 56 - 2026-09-18 - practitioner-profile Phase H Task H4, independent cross-family codex peer review (confirmation round after round 55, pass stage-6.p1)

- Round status: Closed — No new findings — converged
- Source: Codex peer-review
- Baseline: HEAD `bc6e2f0b72ccc830a1d500d45a6e3bd827104659`; working-tree LEG-2 fixes against HEAD.
- Files reviewed: The six scoped files: `desktop/src/scribe_desktop/ui/microphone.py`, `desktop/src/scribe_desktop/ui/models.py`, `desktop/tests/test_ui_screens.py`, `desktop/tests/test_status_and_app.py`, `desktop/tests/test_integration_no_sockets.py`, and `docs/security/data-flow-map.md`; targeted downstream reads checked root propagation, resolver lookup and profile loading. Round 55 supplied the findings and fix decisions.
- Validation basis: Static source/diff review and caller searches only; no files written, project imports or tests run. Composer-reported ruff clean, mypy strict clean over 34 files and pytest 2023 passed were not independently rerun. Bounded `git diff --stat` shows 16 files, 534 insertions and 42 deletions, consistent with the recorded hardening work and fixes; a HEAD-relative stat cannot independently prove other hunks unchanged since round 55 without that working-tree snapshot.
- Finding verification: 0 candidates / 0 dropped / 0 downgraded.

#### Confirmed / disputed (round 55)

- **PR-LOW-041 — confirmed fixed:** The helper supplies temporary profile/config roots at `desktop/tests/test_ui_screens.py:727` and `:730`, covering all 20 previously enumerated callers; `"kwargs.update(overrides)"` at `:738` preserves explicit overrides. The offscreen smoke supplies both roots at `desktop/tests/test_status_and_app.py:71`; the child derives sessions/profile/config from one temporary parent at `desktop/tests/test_integration_no_sockets.py:215`. The new pin poisons both actual resolver symbols at `desktop/tests/test_ui_screens.py:821` and `:822` and exercises construction plus both refreshes. Its `AssertionError` escapes the narrower profile/config error handlers (`desktop/tests/test_ui_screens.py:704`; `desktop/src/scribe_desktop/ui/practitioner.py:781`).
- **PR-REG-006 — confirmed fixed:** The `"present != self._profile_line_model_present"` guard at `desktop/src/scribe_desktop/ui/microphone.py:310` skips profile reads during steady polls. Recording presence before rereading and rendering (`:325`) terminates the refresh cycle after one read for a stable install/removal transition. The renderer recomputes the model-dependent reason (`desktop/src/scribe_desktop/ui/models.py:1375`) and caches only its string (`desktop/src/scribe_desktop/ui/microphone.py:326`), retaining no biometric vector. Read counts explicitly cover installation, steady presence, removal and steady absence at `desktop/tests/test_ui_screens.py:559`, `:564`, `:567` and `:569`. Precision on absent profiles: a presence flip still invokes the absence probe; `desktop/src/scribe_desktop/practitioner_profile.py:313` returns before key unwrap/decryption. The revised microphone docstrings, report docstring and flow 12 describe this transition-only exception consistently.

PEER-ROUND-56 RESULT: 0 findings (CRIT 0 / HIGH 0 / MED 0 / LOW 0).
