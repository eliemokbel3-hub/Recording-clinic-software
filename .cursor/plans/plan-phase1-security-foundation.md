...
- **The host manifest `path` MUST be an `.exe`** — current Chrome (verified on 150.x) silently refuses to launch `.bat`/`.cmd` native messaging hosts: no process is spawned, the extension just sees a disconnect. Every working native host on a normal Windows machine points at an executable. We therefore register `.venv\Scripts\scribe-host.exe`, generated by pip from the **gui-scripts** entry point (windowless, pythonw-backed, receives Chrome's bare origin argv plus `--parent-window` directly). Chrome spawns it with an arbitrary cwd and no console — the integration test reproduces exactly that (no shell, `CREATE_NO_WINDOW`, foreign cwd). Rerun registration after any venv move; `pip install -e desktop` must have run first so the exe exists
...
- [x] 🟩 Step 6: Registration tooling — done 2026-07-24; REVISED 2026-07-25 at the Step 12 gate: the `.bat` launcher was the reason Chrome never spawned the host (current Chrome refuses .bat/.cmd native hosts — no error surfaced, just a disconnect). Now registers `.venv\Scripts\scribe-host.exe` (gui-scripts entry point, windowless); `.bat` generation removed, legacy copies deleted on register/unregister; HKCU write verified; integration test now spawns the exe the way Chrome does
