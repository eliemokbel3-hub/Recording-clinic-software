...
4. Score R1–R6 on the sheet from the Note tab, BEFORE leaving the review: the tab's plaintext is cleared on close, on Complete and on Discard, and a cancelled review is gone. Timing for R6 (*proposed*) starts at Generate and stops when every proposal is decided and every warning acknowledged.
...
The sheet is the Task 9.1 table in the plan (`.cursor/plans/plan-phase3a-note-pipeline.md`, under Task 9.1): one row per consultation with the R1–R6 columns, a totals row, and a decision line — date, rubric version, set id, pass or fail. Counts and yes/no only; never a transcript or note line, a name, or a quote.

## What is measured

Scored per note during review, as counts and yes/no only. An "assertion" is one bullet of the note (the Note tab renders one bullet per assertion, never assembled prose), so counts are unambiguous.
