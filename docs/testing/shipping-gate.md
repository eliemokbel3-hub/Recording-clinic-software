...
**Status: RATIFIED — rubric v1, 2026-09-05.** Drafted as Phase 3A Task 9.1a (2026-09-04); the practitioner ratified the proposed composition, rubric and pass rule UNCHANGED on 2026-09-05 (in session: "use suggested rules"), before any scoring. The numbers below are frozen for this run and are never adjusted after; a different rule would be a new rubric version, decided before its own run. **Run status (2026-09-05): PAUSED by practitioner decision until `.cursor/plans/plan-practitioner-profile.md` ships voice enrolment (its Phase 3), so one recording set serves this gate, Task 2.3 and the enrolment measurement; the rubric is unchanged.** That plan's Task 5.5 clarified two things on this page on 2026-09-16, before recording starts: R3's population once lines can be removed during review ("What is measured", below), and the mock definition — a second real person speaks the patient part, because enrolment recognises a person, not a part ("Preparing the transcript set"). Both are clarifications of rubric v1, not threshold changes; the practitioner initials the R3 clarification where marked. This page carries no transcript or note text, and neither does the scoring sheet: scores are counts and yes/no only.
...
Each consultation is MOCK — mock content, no real patient — the ratified default, unless the practitioner expressly substitutes a consented recording for a given consultation. Clarified at practitioner-profile Task 5.5 (2026-09-16, per that plan's Task 6.1 protocol): the practitioner speaks ONLY the clinician part and a second real person (a colleague or family member) speaks the patient part, with a third real voice for the three-speaker consultation. One person acting both parts — the Phase 2 completion gate's mocks — cannot be scored for speaker separation or for the voice profile's auto-confirm, because enrolment recognises a person, not a part; this set is therefore not acted solo. Retention is the app's own: the session's audio, transcript and note live under its session key and are destroyed cryptographically at Complete or Discard. A live session awaiting review is sweep-exempt, so the practitioner's Complete or Discard is what ends it; a session left behind — the app closed or crashed before Complete or Discard — becomes an unprotected recovery store, expiry-eligible at 24 h from its creation and destroyed by the next successful sweep: an eligibility bound, not a deadline (`docs/security/retention-schedule.md`). A set kept for re-use at Phase 3B's gate must be mock and falls under the Task 2.3 retention decision, still open in the plan.

Composition — ratified 2026-09-05 (rubric v1):

- 10 consultations (8 to 12 accepted) spanning new-patient and follow-up encounters.
- At least three body regions, each with one prefill seed in the practitioner's config, so prefill is exercised once per region. The run's config (three prefills, one trigger, and the run's `section_cues.json` — the cue file that routes transcript utterances into sections, a byte-identical copy of the shipped default by the practitioner's decision of 2026-09-16: their own phrasing is to accrue through phrase learning, practitioner-profile plan Phase 5, not be authored for the gate) is kept in `docs/testing/shipping-gate-config/`, with install steps in its README.
...
- **R3 noise** = assertions that must be deleted / assertions in the note.
  ...
  **R3 with Remove available (clarified 2026-09-16, practitioner-profile Task 5.5 — a clarification of rubric v1; the 20 % threshold and every other number are untouched).** Since the Note tab gained Remove line / Move / Undo (its "Edit the note" group), a line the provider put in the note can be gone by the scoring point. The population is fixed as follows:
  - **Denominator** = the assertions in the note at the scoring point BEFORE any removal but AFTER every proposal decision: the provider's transcript lines plus the confirmed proposals — the population rubric v1 already scores. A removal does not shrink it.
  - **Numerator** = the EFFECTIVE removals — removals still in force at the scoring point (an undone removal does not count; a move's removal leg does not count, because the line is still in the note) — plus the lines still in the note that would need deletion at signing. A removal made, undone and made again counts once.
  - Lines the practitioner ADDED by hand count in neither: they are the practitioner's decision, not the provider's noise. (For R1, a line the practitioner MOVED counts as wrongly routed — it was moved because the router placed it wrongly — exactly as it would have been tallied before Move existed.)
  - Worked examples, text-free: (1) proposal decisions only, no edit — 6 transcript lines and 2 confirmed proposals, nothing removed, 1 line still needing deletion → R3 = 1/8. (2) remove → undo — 6 transcript lines and 1 confirmed proposal, one line removed then undone before the scoring point, nothing needing deletion → R3 = 0/7. (3) a move — 6 transcript lines and 1 confirmed proposal, one line moved to another section (its removal leg does not count) and one other line removed and left removed → R3 = 1/7. (4) a proposal-only note — no transcript line routed at all, 3 confirmed proposals, 1 of them still needing deletion at signing → R3 = 1/3 (the denominator is the confirmed proposals alone; nothing to remove, since Remove acts on transcript lines).
  - Clarification initialled by the practitioner: ________ (date ________).
