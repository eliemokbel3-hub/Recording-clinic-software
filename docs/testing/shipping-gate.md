...
**Status: RATIFIED — rubric v1, 2026-09-05.** Drafted as Phase 3A Task 9.1a (2026-09-04); the practitioner ratified the proposed composition, rubric and pass rule UNCHANGED on 2026-09-05 (in session: "use suggested rules"), before any scoring. The numbers below are frozen for this run and are never adjusted after; a different rule would be a new rubric version, decided before its own run. **Run status (2026-09-05): PAUSED by practitioner decision until `.cursor/plans/plan-practitioner-profile.md` ships voice enrolment (its Phase 3), so one recording set serves this gate, Task 2.3 and the enrolment measurement; the rubric is unchanged.** That plan's Task 5.5 clarifies two things on this page before recording starts: R3's population once lines can be removed during review, and the mock definition — a second real person speaks the patient part, because enrolment recognises a person, not a part. This page carries no transcript or note text, and neither does the scoring sheet: scores are counts and yes/no only.
