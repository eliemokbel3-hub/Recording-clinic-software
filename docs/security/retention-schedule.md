...
| ML model cache (silero ~2 MiB; runtime default `whisper\medium` ~1.43 GiB; fallback `whisper\small` ~465 MiB; the speaker-embedding model `speaker-embedding\wespeaker-voxceleb-resnet34-LM.onnx` ~25 MiB; ~3.0 GiB with all four benchmark candidates) | `%LOCALAPPDATA%\ClinikoScribe\models\` | Indefinite — static program data, NO clinical content | Manual delete at any time; re-created by `scripts/setup-models.py` (run by the user, setup-time network only) |
| Practitioner voice profile (`voice.enc` + its `key.dpapi`; practitioner-profile plan Phases 1–3) | `%LOCALAPPDATA%\ClinikoScribe\profile\` | Indefinite, until the practitioner deletes or re-records it on the Practitioner tab — deliberately OUTSIDE the 24 h rule and never swept: it is the practitioner's own data, not a patient's. Content: a numeric voice vector (never audio — `record_enrolment` returns PCM to memory only and clears its buffers on every exit, and the tab's worker drops its reference once `enrol` returns), the embedder identity (`model_id`, `model_sha256`), creation time, speech seconds, the device name and the consent record; AES-256-GCM under a key that is DPAPI-wrapped (current-user) with a profile-specific description the unwrap verifies, so a session key can never open it nor it a session. Re-enrolment replaces ONLY `voice.enc` under the existing key, and so does "Confirm consent" (practitioner-profile plan Phase 5: the same vector re-saved with a current consent record). **Consent record** (`accepted_at`, `consent_text_version` — the ratified text's version: `consent-v1` for records saved before Phase 5, `consent-v2` since; an older version is readable but not current, so the tab asks for a fresh tick and phrase learning stays off until re-consent — and `learning_opt_in`, as ticked when the record was saved): lives INSIDE `voice.enc` and nowhere else, so it shares the blob's lifetime and custody exactly — written at each (re-)enrolment or re-consent, deleted with the key; it is the version string and time, not a copy of the text | `practitioner_profile.delete_profile`, reached from the tab's confirmed Delete: key unlinked FIRST (cryptographic deletion), then the blob; idempotent. Plain NTFS unlink — the same forensic residual as session custody, accepted at the same-user boundary (threat model). The profile's field names are log-tripwire markers, so a repr, full dump or JSON of it carries a marker and is dropped by the last-line filter (the tripwire's documented limit — bare values with no field name attached — is unchanged; the primary control is that nothing in the profile path logs) |
| Clinician-authored config (`template_profiles.json`, `autofill_rules.json`, `prefill_templates.json`, Phase 3A; `section_cues.json` — the phrases that route a transcript utterance into a note section — since the practitioner-profile plan's Phase 4) | `%LOCALAPPDATA%\ClinikoScribe\config\` | Indefinite — INTENDED as non-patient boilerplate; deliberately OUTSIDE the encrypted session store and the 24 h rule so it survives session destruction | Manual delete/edit, plus — for the cue file only — the Practitioner tab's Delete of a learned phrase (below); the loader is read-only and never creates or deletes config. Patient data and secrets are prohibited by policy, but the loader validates only STRUCTURE (shape, length, control/format characters, atomic-claim shape, no duplicate cue phrase) — it cannot detect semantic misuse, so any content a clinician hand-edits in is retained VERBATIM in plaintext. Autofill and prefill text feeds the note pipeline only as proposals; cue phrases never enter a note — they choose which verbatim transcript utterance lands in which section |
| Learned phrases (practitioner-profile plan Phase 5, D9 as amended): entries appended to the user `section_cues.json` by `note_config.append_user_cues`, plus the sidecar `section_cues.learned.json` (`{phrase: {section, learned_at}}`, read only by the Practitioner tab) | `%LOCALAPPDATA%\ClinikoScribe\config\` | Indefinite, until the practitioner deletes the phrase on the Practitioner tab (or edits the file) — plain text, the practitioner's own words: two to four normalised content tokens from the START of a line the CONFIRMED clinician added or moved during review, written only when that note was SAVED, only under a readable profile carrying the CURRENT consent version with the learning opt-in ticked, and only after the refusal filter (`refuse_learning_candidate`: names, numbers, dates, medication names — a SHAPE filter, not a meaning filter) passed every source token; a patient's line never reaches the learner (the ownership rule, pinned) | `note_config.delete_user_cue`, reached from the tab's Delete on either list: the phrase is removed from the cue file and the sidecar, each replaced atomically (cue file first); a delete whose sidecar write failed is finished by the retry (the tab keeps the row), and every delete prunes sidecar entries whose phrase is no longer in the cue file. Whole-file deletion of `section_cues.json` returns the app to the shipped cues and orphans the sidecar, whose entries are then dropped on read. Plain NTFS unlink/replace — the same forensic residual as the other config files, accepted at the same-user boundary |
| Speaker-measurement temporary store (`scripts/measure-speakers.py`, Phase 3A Task 2.3a — a practitioner-run tool, not the app) | `%TEMP%\scribe-speaker-eval-*\<id>\` (`key.dpapi`, `audio.enc`, `transcript.enc`) | Per recording, for the duration of that recording's evaluation only; the source WAV and label files are the practitioner's and are never written, moved or deleted | Torn down key-first at the end of every evaluation path (unlink `key.dpapi` → destroy the in-memory key → remove the session directory → remove the temporary root), every leg attempted whatever the earlier legs did, with a fail-closed `os.stat` probe; teardown returns normally only when the root is positively gone, the key destroyed and no leg failed — otherwise a fault names the path, key state and every leg failure. Residual: a leftover the OS refuses to delete or inspect is NAMED for by-hand removal, never hidden (`speaker_eval.py` module docstring) |
