...
| Practitioner voice profile (`voice.enc` + its `key.dpapi`; practitioner-profile plan Phase 1 — DRAFT row, finalised at that plan's Task 3.3) | `%LOCALAPPDATA%\ClinikoScribe\profile\` | Indefinite, until the practitioner deletes or re-records it — deliberately OUTSIDE the 24 h rule and never swept: it is the practitioner's own data, not a patient's. Content: a numeric voice vector (never audio — `record_enrolment` returns PCM to memory only and clears its buffers on every exit), the embedder identity (`model_id`, `model_sha256`), creation time, speech seconds, the device name and the consent record (`accepted_at`, `consent_text_version`, `learning_opt_in`); AES-256-GCM under a key that is DPAPI-wrapped (current-user) with a profile-specific description the unwrap verifies, so a session key can never open it nor it a session. Re-enrolment replaces ONLY `voice.enc` under the existing key | `practitioner_profile.delete_profile`: key unlinked FIRST (cryptographic deletion), then the blob; idempotent. Plain NTFS unlink — the same forensic residual as session custody, accepted at the same-user boundary (threat model). The profile's field names are log-tripwire markers, so a repr, full dump or JSON of it carries a marker and is dropped by the last-line filter (the tripwire's documented limit — bare values with no field name attached — is unchanged; the primary control is that nothing in the profile path logs) |
