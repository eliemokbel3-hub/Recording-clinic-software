---
name: peer-loop
description: Drive a cross-model-family peer-review loop to convergence — a live executor spawns a headless cross-family peer that runs /peer-review non-interactively, the peer logs findings, the executor verifies and applies them (/fix for code rounds; plan amendments in the /review-plan style in the token-gated plan-review mode), then a fresh peer re-reviews, until the mode's step-4 convergence rule is met, stalemate, or cap 3. Shared Cursor/Codex/Claude Code workflow; invoke as /peer-loop in Cursor or Claude Code, or $peer-loop in Codex. Side-effecting; explicit invocation only.
...
2. **Ask the executor's model family — do not self-detect.** The user is the source of truth (an explicit tool/harness model readout counts as that truth — the probe-or-ask rule of `/peer-review` Step 0 sub-step 5; what is barred is inferring the family from the model's own self-report). Ask whether the live executor (this session) is **Claude-family** or **GPT-family**. Never infer it from the model's own self-report.
...
2. **Spawn the cross-family peer headless under the non-interactive contract.** Invoke the chosen CLI with a prompt whose FIRST nonblank line is the control header — exactly the sentinel token **`PEER_LOOP_NONINTERACTIVE`** (code mode) or both tokens (plan-review mode) — followed by the instruction to run `/peer-review` (`$peer-review` in Codex) and the composer-supplied scope; token text anywhere else in the prompt is inert content (`/peer-review` reads the mode from the control header alone). The token activates `/peer-review`'s non-interactive headless contract: suppress the proceed prompt and the model-diversity warning, force path (b) always-log, force a fresh review of the supplied scope in the token-selected mode (CODE review by default — never plan-review mode without the plan token, never a stale prior round; passing `PEER_LOOP_PLAN_REVIEW` alongside the sentinel selects headless PLAN review instead — plan-only scope; the plan token alone is INVALID and `/peer-review` refuses loudly, writing nothing), and set the correct mode-distinct `Source:`. Example (Claude-family executor → `codex` peer):
   ...
   The peer appends a Round block to the plan's `Review Findings Log` and exits. Do not hand-edit its findings. Before proceeding to step 3, confirm the round block actually landed: a peer run that wrote no round (e.g. its pair-integrity pre-check failed closed, writing nothing — or it surfaced a valid-but-unrecovered pair and held its write) is an ERROR to surface with the peer's reason; skip step 3 (there is nothing to ingest) and treat the round as inconclusive — never as zero findings, convergence, or stalemate.
...
3. **The live executor verifies each finding and applies `/fix` (code rounds; in plan-review mode, the planning session applies `/review-plan`-style amendments instead).** Because this session holds the execution context, run `/fix` here interactively with all its gates: `/fix` picks up the new peer round via its Step 0 Findings Log path, and — since this is the live `/peer-loop` executor — verifies each finding against the code (session memory catches peer false positives) before applying, one at a time, with stop-on-failure (independent mechanical LOWs may batch per `/fix` Step 2's scoped exception). Deferral Confirmation Gate, CRIT/HIGH scope-expansion hard-stops, and `/fix` stop-on-failure all PAUSE the loop for the user; the loop never auto-resolves them. **Plan-review mode routes differently:** `/fix` never ingests a `… plan peer-review` round (its Step 0 predicate excludes them) — instead the planning session driving this loop verifies each plan finding against the plan + the code it names (including its `Materiality:` label, per the plan-mode enum canonically defined in `/peer-review`'s plan-review mode) and applies the accepted amendments `/review-plan`-style (or via a scoped `/review-plan`), marking per-finding dispositions on the round, applying each amendment under `/review-plan`'s **Amendment-application discipline** (the canonical definition; do not restate it); the same pause gates apply unchanged.

4. **Termination check** (after the fix — or, in plan-review mode, the amendment pass; a wrote-nothing peer was already surfaced at step 2's appended-round confirmation and never reaches this check). Code rounds and plan rounds converge differently; the plan-mode materiality rule changes nothing about code-mode termination:
   - **Converged → stop, success (code rounds).** If this round's peer returned zero verified findings, the loop is done — report the rounds run and the final round state, and remind the user to run the project's test suite or build command (peer review and per-fix verification cannot catch everything).
   - **Converged → stop, success (plan rounds — materiality-based).** In plan-review mode the loop converges when a round from round 2 onward yields zero NEW `build-affecting` findings, judged on the owning session's VERIFIED labels (post-reclassification) — materiality per the plan-mode enum canonically defined in `/peer-review`'s plan-review mode (the canonical definition; do not restate it). Mandatory guardrails: **never converge on round 1** — even a zero-finding first round does not converge: it proceeds to round 2 when the cap permits (a cap of 1 still stops with a report, unconverged); one clean peer pass is too weak an all-clear for a pre-build plan; **`record-only` findings are still applied** — batched into this round's amendment pass, never dropped; they do not extend the loop, but the round is not converged until they have landed; **strict override** — a per-invocation strict-convergence request (stated at invocation the same way a "max N" cap override is; a composer-supplied configuration counts as the invocation) restores zero-findings convergence — the loop then stops only on a round where no finding survived verification, `build-affecting` and `record-only` alike (an `invalid`-classified finding did not survive verification and never blocks), with the round-1 floor still applying. On plan-mode convergence, report the rounds run and hand the peer-hardened plan back to the planning flow.
   - **Stalemate → stop, report.** If a round produced no *new* verified findings, or there was no code diff (in plan-review mode: no plan diff) since the previous round, stop — further rounds will not converge. Convergence and stalemate in plan-review mode measure the plan diff + finding status, never code diffs; the round-1 floor applies to stalemate too — a first round never converges or stalemates a plan-review loop; only the cap's stop-with-report can end it there.
...
- **Converge on finding presence/verification, not severity counts.** Severity labels are non-deterministic across headless peer runs (the same bug can come back CRIT one run, HIGH the next). Decide convergence and disposition on whether a verified finding is present, not on exact counts (in plan-review mode, per step 4's materiality-based rule and its guardrails; severity counts never decide in either mode).
