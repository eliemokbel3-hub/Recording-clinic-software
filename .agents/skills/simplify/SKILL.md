...
- append a `### Round N — YYYY-MM-DD` block, where N follows `/review`'s round-allocation rule (the canonical definition in its **Detect review round** section; do not restate it); before appending, probe both pair signals and validate the plan+sidecar pair per `/document` Step 6.7's pre-check — the same dual-signal gate `/review` applies; an invalid pair FAILS CLOSED, writing nothing. Also append the one-line `Review History` entry for the round.
