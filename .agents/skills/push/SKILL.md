...
- a Paused plan is NEVER pushed-as-complete: do not treat it as a completed plan, and never offer archive/delete for it or its sidecar
- surface the `Paused since:`/`Paused reason:` fields so the push record reflects the true state; pushing unrelated committed work is unaffected
- resuming is `/load-plan`'s explicit `Paused → Active` transition — never a push side effect
